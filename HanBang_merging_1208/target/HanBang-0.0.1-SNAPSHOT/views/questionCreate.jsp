...
<body onload="showmethe();">
	...
	<section>
		...
		<form action="${ctx}/question/registQuestion.do" method="post">
			<input type="hidden" id="year"> <input type="hidden"
	...
	function showmethe() {
		...
		for (i = 1; i <= m[mm - 1]; i++) {
			...
			if (new Date(yy, mm - 1, i).getDay() == 0) {
				sum += "<td align='center' style='color:red;'>"
						+ "<span onclick='checkDay(" + yy + "," + mm + "," + i
	...
	function checkDay(selectedYear, selectedMonth, selectedDay) {
		...
		if (Number(selectedDate) < Number(today)) {
			...
			if (confirm("선택하신 날짜가 " + selectedYear + "년" + selectedMonth + "월"
				...
				document.getElementById("schedule").value = selectedYear + "/"
